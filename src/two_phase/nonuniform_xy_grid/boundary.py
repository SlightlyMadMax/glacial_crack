...
def init_f_vector(n_x):
    ...
    # Трещина-гауссиана
    F[:] = [20.0 - 5.0 * math.exp(-(i * dx - 0.5) ** 2 / 0.005) for i in range(n_x)]
...
def recalculate_boundary(F, T):
    ...
    h_i = 1.0 - get_y_node_coord(j_int - 1, j_int)
    h_0_i = get_y_node_coord(j_int - 1, j_int) - get_y_node_coord(j_int - 2, j_int)

    h_w = get_y_node_coord(j_int + 1, j_int) - 1.0
    h_1_w = get_y_node_coord(j_int + 2, j_int) - get_y_node_coord(j_int + 1, j_int)
    ...
    for i in range(1, N_X - 1):
        ...
        df_dx = (F[i + 1] * h_x_0 * h_x_0 - F[i - 1] * h_x * h_x + F[i] * (h_x * h_x - h_x_0 * h_x_0)) / (h_x * h_x_0 * (h_x + h_x_0))
    ...
    h = get_x_node_coord(1)
    h_1 = get_x_node_coord(2) - h

    df_dx_0 = ((F[1] - F[0]) * (h + h_1) * (h + h_1) - (F[2] - F[0]) * h * h) / (h * h_1 * (h + h_1))
    ...
    h = 1.0 - get_x_node_coord(N_X - 2)
    h_0 = get_x_node_coord(N_X - 2) - get_x_node_coord(N_X - 3)

    df_dx_n = ((F[N_X - 1] - F[N_X - 2]) * (h_0 + h) * (h_0 + h) + (F[N_X - 3] - F[N_X - 1]) * h * h) / (
                h * h_0 * (h + h_0))
